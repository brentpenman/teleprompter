...
2. Initialize settings on page load (BEFORE voice toggle handler):
- On page load (DOMContentLoaded or module init):
  - Instantiate settingsManager: `const settingsManager = new SettingsManager();`
  - Call load(): `const settings = settingsManager.load();`
  - Apply settings to state (fontSize, scrollSpeed, highlightEnabled, mirrorEnabled)
  - Apply visual settings immediately: font size to CSS, mirror to transform
  - Store settingsManager in module scope for later use

This ensures ENGINE-02 (settings persistence across sessions) by loading settings on page init, not just when voice button clicked.

3. Replace existing voice initialization (currently hardcoded SpeechRecognizer):
...
voiceToggle.addEventListener('click', async () => {
  if (!state.voiceEnabled) {
    ...
    try {
      ...
      if (fallbackReason) {
        // ENGINE-09: Clear error UI when Vosk fails to initialize
        // Check if fallbackReason indicates Vosk initialization failure
        if (fallbackReason.includes('Vosk') || fallbackReason.includes('initialization')) {
          LoadingStates.showError(listeningIndicator,
...
Note: The fallback reason handling explicitly addresses ENGINE-09 by checking if the failure was Vosk-specific and displaying: "Vosk initialization failed: [reason]. Using Web Speech API instead."

4. Add settings button handler:
...
5. Add engine indicator update on state changes
...
2. Verify settings loaded on page init (check console for "Settings loaded" or check applied font size)
...
5. Verify engine indicator appears showing which engine is active
6. If Vosk initialization fails, verify error message shows: "Vosk initialization failed: [reason]. Using Web Speech API instead."
7. Click settings button (or add temporary button if needed)
8. Verify settings panel appears with:
   - Engine selector (radio buttons)
   - Model management (if Vosk supported)
   - Display toggles (highlight, mirror)
9. Change engine preference, close settings
10. Disable/re-enable voice mode
11. Verify new engine used on next initialization
12. Verify script content and scroll position preserved when switching engines
13. Verify all existing features still work (manual scroll, font size, etc.)
...
3. Verify settings persisted (font size, engine preference, toggles)
  ...
  <done>
script.js updated with RecognizerFactory integration and settings loaded on page init. index.html has settings overlay and engine indicator. styles.css has settings panel and loading state styles. Voice mode uses dynamic engine selection. Settings persist across reloads and load on page initialization. Vosk initialization errors display clear messaging before falling back to Web Speech API. All existing features preserved. ENGINE-01, ENGINE-02, ENGINE-04, ENGINE-05, ENGINE-06, ENGINE-07, ENGINE-08, ENGINE-09, INTEG-05 satisfied.
  </done>
</task>
...
2. Verify settings loaded on page init (font size applied, etc.)
3. Paste script text and start teleprompter
4. Click Voice button → Verify engine initialization with loading state
5. Verify engine indicator shows active engine
6. If Vosk fails, verify clear error message displayed
7. Open settings → Verify panel renders
8. Change engine preference → Close settings
9. Disable voice → Re-enable voice → Verify new engine used
10. Reload page → Verify settings persisted
11. Test on different browsers (Chrome, Safari, Firefox) to verify fallback logic
12. Verify manual scroll still works while voice mode active
13. Verify highlight and mirror toggles work in settings
14. Verify model download UI appears when Vosk selected (if supported)
...
- Settings load on page initialization (DOMContentLoaded or module init) for persistence
...
- Vosk initialization failures show clear error message: "Vosk initialization failed: [reason]. Using Web Speech API instead."
...
- Satisfies ENGINE-01, ENGINE-02, ENGINE-04, ENGINE-05, ENGINE-06, ENGINE-07, ENGINE-08, ENGINE-09, INTEG-05
</success_criteria>
